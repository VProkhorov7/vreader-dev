    ...
    private static func performDeleteAll(appGroupID: String?) {
        ...
        for secClass in secClasses {
            ...
            if status != errSecSuccess && status != errSecItemNotFound {
                logger.warning("KeychainManager: deleteAll partial failure class=\(secClass, privacy: .public) status=\(status, privacy: .public)")
            }
        }

        let suffixes = [".webdav", ".smb"]
        for suffix in suffixes {
            var query: [String: Any] = [
                kSecClass as String:              kSecClassGenericPassword,
                kSecAttrService as String:        bundleID + suffix,
                kSecAttrSynchronizable as String: kSecAttrSynchronizableAny
            ]
            if let groupID = appGroupID {
                query[kSecAttrAccessGroup as String] = groupID
            ...
            let status = SecItemDelete(query as CFDictionary)
            if status != errSecSuccess && status != errSecItemNotFound {
                logger.warning("KeychainManager: deleteAll partial failure service=\(suffix, privacy: .public) status=\(status, privacy: .public)")
            }
        }
